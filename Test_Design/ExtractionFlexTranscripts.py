#!/usr/bin/env python3
"""
Flex transcripts parse-only extractor.
...
INPUT_DIR = BASE_DIR / "File" / "Flex" / "flex_transcripts"
...
_LEGAL_RE = [
    ...
    re.compile(r"information herein is based on sources we believe to be reliable", re.IGNORECASE),
]
...
_SPEAKER_LINE_RE = re.compile(
    r"^(?:\*\*)?([A-Z][A-Za-z.&'/-]+(?:\s+[A-Z][A-Za-z.&'/-]+){0,5}|Operator)(?:\s*\([^)]+\))?\s*[:\-]\s*(.*)$"
)
...
def _parse_docling_text(filepath: Path) -> tuple[str, list[dict[str, Any]], list[str]]:
    warnings: list[str] = []
    ...
    try:
        ...
        # Structured table extraction from Docling document object.
        if hasattr(doc, "tables"):
            for i, table in enumerate(doc.tables):
                try:
                    ...
                    tables.append(
                        {
                            "table_id": f"table:docling_{i+1}",
                            "table_type": "financial_statement",
                            "header": [str(c) for c in list(df.columns)],
                    ...
                    warnings.append(f"table_export_error:{i}:{e}")
...
def _parse_source_text(filepath: Path, suffix: str) -> tuple[list[dict[str, Any]], str, list[dict[str, Any]], list[str]]:
    if suffix == ".pdf":
        pages, text, warnings = _parse_pdf_text(filepath)
        return pages, text, [], warnings + ["parser:pypdf"]
    if suffix in {".html", ".htm"}:
        docling_text, docling_tables, docling_warnings = _parse_docling_text(filepath)
        if docling_text:
            return [], docling_text, docling_tables, docling_warnings + ["parser:docling"]
...
def _parse_date_value(value: str) -> datetime | None:
    for fmt in ("%B %d, %Y", "%Y-%m-%d", "%m/%d/%Y", "%m/%d/%y", "%d-%b-%Y"):
        try:
...
def _infer_transcript_section_type(header: str) -> str:
    h = header.lower()
    if any(k in h for k in ["question-and-answer", "questions and answers", "q&a", "question and answer"]):
        return "qa_section"
    if any(k in h for k in ["prepared remarks", "opening remarks", "presentation"]):
...
def _is_likely_speaker_name(value: str) -> bool:
    s = value.strip()
    if not s:
        return False
    if s.lower() == "operator":
        return True
    # Typical person name pattern like "David A. Rubin".
    if not re.match(r"^[A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){1,4}$", s):
        return False
    tokens = [tok for tok in re.split(r"\s+", s) if tok]
    # Reject section-like all-caps tokens (e.g., NEXT-GEN).
    if any(tok.isupper() and len(tok) > 3 for tok in tokens):
        return False
    # Filter common role/section labels that are not speaker names.
    blocked = {
        "president", "officer", "relations", "remarks", "guidance", "highlights",
        "results", "participants", "discussion", "section", "revenue", "margins",
        "solutions", "cash", "flow", "capex", "tax", "spin-off", "automotive",
        "networking", "mobility", "restructuring", "charges", "core", "flex",
        "question", "answer", "financial", "reliability", "agility", "overview",
        "pages", "total", "copyright", "formatted", "report",
    }
    lowered = s.lower()
    return not any(token in lowered for token in blocked)


def _extract_speaker_turns(lines: list[str], fallback_speaker: str = "") -> list[dict[str, Any]]:
    ...
    def _flush() -> None:
        nonlocal current_speaker, current_role, current_turn_type, current_lines
        text = _clean_text("\n".join(current_lines))
    ...
    role_keywords = {"vice president", "chief", "analyst", "officer", "director", "president"}
    ...
    for idx, line in enumerate(lines):
        ...
        if not line or line.strip() == "---":
            continue
        if _MD_H2_RE.match(line) or _MD_H3_RE.match(line):
            continue

        # Handle corrected transcript format where speaker name is on its own line
        # and the following line contains the role/title.
        stripped = line.strip()
        next_line = lines[idx + 1].strip() if idx + 1 < len(lines) else ""
        next_line_clean = next_line.rstrip()
        next_line_ends_aq = next_line_clean.endswith(" A") or next_line_clean.endswith(" Q")
        if _is_likely_speaker_name(stripped) and (
            any(k in next_line.lower() for k in role_keywords)
            or next_line_ends_aq
        ):
            _flush()
            current_speaker = stripped
            # pypdf format may include role and turn type in one line, e.g. "Chief Financial Officer A".
            role_turn_match = re.match(r"^(.+?)\s+([AQ])\s*$", next_line)
            if role_turn_match:
                current_role = role_turn_match.group(1).strip()
                current_turn_type = "answer" if role_turn_match.group(2) == "A" else "question"
            else:
        ...
        if m:
            candidate = m.group(1).strip()
            if not _is_likely_speaker_name(candidate):
                if current_speaker:
    ...
    # If no explicit "Speaker: ..." line is present, use section speaker fallback.
    if not turns and fallback_speaker and _is_likely_speaker_name(fallback_speaker):
        joined = _clean_text("\n".join(lines))
        if joined:
            turns.append(
                {
...
def _build_sections_from_items(lines: list[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    if not headings:
        # For pypdf transcript output, use all-caps lines as section delimiters.
        _ALLCAPS_SECTION_RE = re.compile(r"^([A-Z][A-Z\s&,./()-]{5,}?)(?:\s*\.{3,})?\s*$")
        # Ignore noisy all-caps disclaimer lines by limiting header length.
        _MAX_HEADER_LEN = 80

        cap_headings: list[dict[str, Any]] = []
        for i, line in enumerate(lines):
            s = line.strip()
            ...
            if re.match(r"^[\.\s]+$", s) or re.match(r"^\d+$", s):
                continue
            m = _ALLCAPS_SECTION_RE.match(s)
            if m:
                clean_header = re.sub(r"\.{3,}", "", m.group(1)).strip()
        ...
        if not cap_headings:
            # No structure detected: keep whole transcript in one section.
            content_raw = "\n".join(lines).strip()
            content_clean = _clean_text(content_raw)
            turns = _extract_speaker_turns(lines)
            sections.append(
                {
                    "section_id": "s1",
                    "header": "Transcript",
                    "level": 1,
                    "item_code": "",
                    "section_type": "transcript_section",
                    "is_tail": False,
                    "content": content_raw,
        ...
        # Use all-caps headings and fall through to the existing heading pipeline.
        headings = cap_headings

    if headings[0]["start"] > 0:
        preface_lines = [ln for ln in lines[: headings[0]["start"]] if not _PAGE_NUM_RE.match(ln)]
        preface_legal = [ln for ln in preface_lines if _is_legal_line(ln)]
        preface_kept = [ln for ln in preface_lines if not _is_legal_line(ln)]
        preface = "\n".join(preface_kept).strip()
        if preface:
            turns = _extract_speaker_turns(preface_kept)
            legal_lines_all.extend(preface_legal)
            sections.append(
                {
                    "section_id": "s0",
                    "header": "Overview",
                    ...
                    "has_legal_boilerplate": bool(preface_legal),
                    "legal_boilerplate_lines": preface_legal,
                    "speaker_turns": turns,
                    "speaker_turn_count": len(turns),
                    "speakers": sorted({t["speaker"] for t in turns}),
                    "parent_section_id": "",
    ...
    for i, hd in enumerate(headings):
        ...
        # Track latest speaker heading so subsequent content-only sections can inherit it.
        section_type = _infer_transcript_section_type(hd["header"])
        if _is_likely_speaker_name(hd["header"]):
            current_speaker_section = hd["header"]
        elif section_type == "qa_section":
            # Reset speaker inheritance when entering Q&A to avoid misattributing analyst prompts.
            current_speaker_section = ""

        content_raw = "\n".join(kept).strip()
        content_clean = _clean_text(content_raw)
        turns = _extract_speaker_turns(kept, fallback_speaker=hd["header"])
        if not turns and content_raw and current_speaker_section:
            turns = [
                {
                    "speaker": current_speaker_section,
                    "text": content_raw,
                    "char_count": len(content_raw),
                }
            ]
...
def _build_full_text_core(sections: list[dict[str, Any]]) -> str:
    ...
    for s in sections:
        ...
        turns = s.get("speaker_turns") or []
        if turns:
            for t in turns:
                speaker = str(t.get("speaker", "")).strip()
                text = str(t.get("text", "")).strip()
                if speaker and text:
                    core.append(f"{speaker}: {text}")
            continue
    ...
    for s in sections:
        ...
        if turns:
            for i, t in enumerate(turns, start=1):
                ...
                if not txt:
                    continue
                tid = f"{s['section_id']}:t{i}"
                content = f"{speaker}: {txt}" if speaker else txt
                blocks.append({
                    "block_id": tid,
                    "type": "speaker_turn",
                    "section_id": s["section_id"],
                    ...
                    "turn_type": str(t.get("turn_type", "") or "").strip(),
                    "content": content,
                    "char_count": len(content),
                    "is_tail": s.get("is_tail", False),
                })
                reading_order.append({"order": order, "type": "speaker_turn", "ref": tid, "page_num": 0, "preview": content[:120]})
                order += 1
            continue
...
def _basic_classification(rel_path: Path, file_name: str, full_text: str) -> dict[str, Any]:
    ...
    if "transcript" in text:
        signals.append("transcript_keyword")
    if "earnings call" in text:
        signals.append("earnings_call_keyword")
    if "question-and-answer" in text or "q&a" in text:
        signals.append("qa_keyword")
...
def parse_document(filepath: Path, rel_path: Path) -> dict[str, Any]:
    ...
    pages, full_text, parsed_tables, warnings = _parse_source_text(filepath, suffix)
    ...
    tables: list[dict[str, Any]] = list(parsed_tables)
    if exhibits:
        tables.append(
            {
    ...
    if classification.get("doc_subtype") == "earnings_call_transcript":
        text_head = full_text[:500]
        date_match = re.search(r"\d{2}-[A-Za-z]{3}-\d{4}", text_head)
        company_match = re.search(r"(Flex Ltd\.?)\s*\(([A-Z]+)\)", text_head)
        quarter_match = re.search(r"(Q\d\s+\d{4}\s+Earnings Call)", text_head)

        date_val = date_match.group(0) if date_match else ""
        name_val = company_match.group(1) if company_match else ""
        title_val = quarter_match.group(1) if quarter_match else ""

        cover_metadata["fields"]["date_of_report"] = date_val
        cover_metadata["fields"]["date_of_report_raw"] = date_val
        cover_metadata["fields"]["date_of_report_normalized"] = date_val
        cover_metadata["fields"]["registrant_name"] = name_val
        cover_metadata["fields"]["registrant_name_raw"] = name_val
        cover_metadata["fields"]["registrant_name_normalized"] = name_val
        cover_metadata["fields"]["title"] = title_val

    return {
        "source": {
            "doc_type": "flex_transcript_only",
            "suffix": suffix,
...
def _prepare_output_dir() -> Path:
    return prepare_output_root(
        OUTPUT_PARENT_DIR,
        legacy_marker_dirs=("flex_transcripts",),
        current_dir_patterns=("flex_transcripts", "flex_transcripts_*"),
    )
...
def extract_all(input_dir: Path = INPUT_DIR, output_dir: Path | None = None) -> tuple[Path, int, int, int]:
    ...
    out_transcripts = output_dir / f"flex_transcripts_{run_stamp}"
    ...
    for src in files:
        ...
        tdir = out_transcripts / rel.parent
        ...
        try:
            ...
            if not sample_written:
                shutil.copy2(out_file, sample_dir / out_file.name)
                sample_written = True
        except Exception as e:
            failed += 1
    ...
    # Fallback for edge cases where sample was not copied during loop.
    if not sample_written:
        first_sample = next(iter(sorted(out_transcripts.rglob("*.parsed.json"))), None)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Flex transcripts parse-only extractor")
    ap.add_argument("--input-dir", type=str, default=None, help="Input dir (default: Test_Design/File/Flex/flex_transcripts)")
    ...
    print("=" * 60)
    print("TEST DESIGN FLEX TRANSCRIPTS PARSE-ONLY EXTRACTION")
    print("=" * 60)
