#!/usr/bin/env python3
"""
Press Release parse-only extractor.
...
INPUT_DIR = BASE_DIR / "File" / "Flex" / "Press Releases"
...
_LEGAL_RE = [
    ...
    re.compile(r"forward.looking statements within the meaning of", re.IGNORECASE),
    re.compile(r"readers are cautioned not to place undue reliance", re.IGNORECASE),
]
...
def _build_sections_from_items(lines: list[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    for idx, hd in enumerate(headings, start=start_id):
        ...
        has_legal = bool(legal)
        if has_legal:
            # If a section is a legal disclaimer block, route the entire section body to legal lines.
            legal = body
            kept = []
        else:
    ...
    for s in sections:
        ...
        if header in noise_headers:
            if not content:
                continue
            s["header"] = "Notes and Disclosures"

        # Drop empty parent (##) sections; keep detailed child sections.
        if level == 2 and not content:
            continue
        filtered_sections.append(s)

    # Keep section IDs compact and deterministic after filtering.
    for i, s in enumerate(filtered_sections, start=1):
        s["section_id"] = f"s{i}"

    return filtered_sections, exhibits, legal_lines_all
...
def parse_document(filepath: Path, rel_path: Path) -> dict[str, Any]:
    ...
    if classification.get("doc_subtype") == "press_release":
        text_head = full_text[:800]
        pr_date_match = re.search(
            r"[A-Za-z\s,]+,\s+([A-Za-z]+\s+\d{1,2},\s+\d{4})\s*[-\u2013\u2014]?",
            text_head,
        )
        pr_title_match = re.search(
            r"(FLEX\s+REPORTS\s+.+?RESULTS)",
            text_head,
            re.IGNORECASE,
        )
        pr_name_match = re.search(
            r"^([A-Z][A-Z]+)\s+REPORTS\b",
            text_head,
            re.MULTILINE,
        )

        date_val = pr_date_match.group(1) if pr_date_match else ""
        title_val = pr_title_match.group(1).strip() if pr_title_match else ""
        name_val = pr_name_match.group(1).title() if pr_name_match else ""
        if not name_val and pr_title_match:
            name_val = pr_title_match.group(1).split()[0].title()
        ...
        if date_val:
            ...
            cover_metadata["fields"]["date"] = date_val
        if name_val:
...
def _prepare_output_dir() -> Path:
    return prepare_output_root(
        OUTPUT_PARENT_DIR,
        legacy_marker_dirs=("press_releases",),
        current_dir_patterns=("press_releases", "press_releases_*"),
    )
...
def extract_all(input_dir: Path = INPUT_DIR, output_dir: Path | None = None) -> tuple[Path, int, int, int]:
    ...
    out_press = output_dir / f"press_releases_{run_stamp}"
    ...
    if not sample_written:
        first_sample = next(iter(sorted(out_press.rglob("*.parsed.json"))), None)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Press Release parse-only extractor")
    ap.add_argument("--input-dir", type=str, default=None, help="Input dir (default: Test_Design/File/Flex/Press Releases)")
    ...
    print("=" * 60)
    print("TEST DESIGN PRESS RELEASE PARSE-ONLY EXTRACTION")
    print("=" * 60)
